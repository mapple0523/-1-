...
io.sockets.on("connection", function (socket) {
    ...
    socket.on("login", function (data) {
        ...
        if (!loginuser.includes(data.userId)) {
            loginuser.push(data.userId)
        }
    ...
    socket.on("send", function (data) {
        ...
        if(loginuser.includes(data.receive)) {
            let user01Socketid = clientSocketMap[data.receive].socketid
            //받는사람에 해당하는 receive즉 받는사람의 socketId를 찾아서
            //현재 받는사람의 id에 해당하는 소켓이 없다면 오류가 발생함
            //오류가 발생하여서 data.receive의 값이 loginuser에 있을때만작동하도록 바꾸엇고
            //로그인 유저가없을때는 alert로 알려주도록 하였음음
